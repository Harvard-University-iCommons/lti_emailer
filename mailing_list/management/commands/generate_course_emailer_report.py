...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--school", required=True, help="Canvas account school id (e.g. hls)"
        )
        parser.add_argument(
            "--academic-years",
            nargs="+",
        ...
        parser.add_argument(
            "--output-file", help="File to output mailing list details to"
        )
    ...
    def handle(self, *args, **options) -> None:
        ...
        output_file = (
            ...
            else "./mailing_list/management/commands/report.csv"
        )
        ...
        canvas_course_id_list = list(
            MailingList.objects.filter(
                date_created__gte="2022-01-01 00:00:00"
        ...
        course_instances = CourseInstance.objects.filter(
            course__school_id=school,
            term__academic_year__in=academic_years,
        ...
        if course_data:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"CSV report has been generated and saved to {output_file}"
                )
            )
        else:
            self.stdout.write(
                self.style.ERROR("ERROR CSV report has NOT been generated.")
            )
