...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for term_id in options["enrollment_term_id"]:
            try:
                courses += (
                    get_courses_for_account_in_term(options["account_id"], term_id)
                    or []
                )
        ...
        for course in courses:
            ...
            for ml in lists:
                ...
                if is_primary(ml):
                    ...
                    secondary.append(ml)
            course_lists[course["id"]] = {"primary": primary, "secondary": secondary}

        if failures:
            for course_id, error_message in failures.items():
                logger.error(
                    "Unable to get/create mailing lists for canvas course id {}: {}".format(
        ...
        self.stdout.write(
            "Total of {} list(s) for {} course(s)".format(num_lists, len(courses))
        )
        ...
        if options.get("output_file"):
            ...
            for course_id in sorted(course_lists):
                ...
                primary = ";".join(
                    [l["address"] for l in course_lists[course_id]["primary"]]
                )
                secondary = ";".join(
                    [l["address"] for l in course_lists[course_id]["secondary"]]
                )
                row = (
                    str(course_id),
