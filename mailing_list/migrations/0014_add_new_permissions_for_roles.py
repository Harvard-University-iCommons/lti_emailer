...
PERMISSION_NAMES = ["lti_emailer_send_all", "lti_emailer_view"]
...
NOT_ALLOWED_ROLES = ["Enrollee", "Petitioner", "Waitlisted", "Course Assistant"]


def create_permissions(apps, schema_editor):
    permission_class = apps.get_model("lti_permissions", "LtiPermission")

    # Make sure to rename all Course Head roles to Head Instructor prior to creating the new permissions
    permission_class.objects.filter(canvas_role="Course Head").update(
        canvas_role="Head Instructor"
    )
    ...
    for old_permission in all_permissions:
        try:
            ...
            for new_role in new_roles:
                permission_class(
                    permission=old_permission.permission,
                    canvas_role=new_role,
                    school_id=old_permission.school_id,
                    allow=old_permission.allow,
                ).save()
        except KeyError:
    ...
    for role in NOT_ALLOWED_ROLES:
        for permission in PERMISSION_NAMES:
            permission_class(
                permission=permission, canvas_role=role, school_id="*", allow=False
            ).save()
...
class Migration(migrations.Migration):
    dependencies = [
        (
            "mailing_list",
            "0013_create_course_settings_model_and_update_mailing_list_date_fields",
        ),
    ]
