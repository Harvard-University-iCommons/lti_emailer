...
def admin_index(request):
    ...
    logger.info(
        "Rendering mailing_list admin_index view for user %s", logged_in_user_id
    )
    return render(
        request,
        "mailing_list/admin_index.html",
        {"course_name": course_name, "build_info": build_info},
    )
...
def list_members(request, section_id=None):
    ...
    if section_id:
        mailing_list = get_object_or_404(
            MailingList, canvas_course_id=canvas_course_id, section_id=section_id
        )
    else:
        mailing_list = get_object_or_404(
            MailingList, canvas_course_id=canvas_course_id, section_id__isnull=True
        )
    ...
    return render(
        request,
        "mailing_list/list_details.html",
