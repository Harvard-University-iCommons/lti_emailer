...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        logger.info(
            "Creating Canvas user primary email report for term_id %d...", term_id
        )
        ...
        for ci in CourseInstance.objects.filter(term=term):
            if ci.canvas_course_id:
                logger.info(
                    "Retrieving users for canvas_course_id %d", ci.canvas_course_id
                )
                users = {}
                try:
                    ...
                    logger.exception(
                        "Failed to retrieve users for canvas_course_id %d",
                        ci.canvas_course_id,
                    )
                    continue
        ...
        logger.info(
            "User primary email report complete for term_id %d at %s",
            term_id,
            report_path,
        )
