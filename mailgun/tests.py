...
from mailgun.route_handlers import CommChannelCache, handle_mailing_list_email_route
...
class RouteHandlerUnitTests(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            email="unittest@example.edu", password="insecure", username="unittest"
        )
    ...
    def test_response_exception(self, mock_handle_recipient, mock_log_exc):
        ...
        Mailgun.
        """
        raised_response = JsonResponse({"message": "I'm a teapot!"}, status=418)
    ...
    def test_missing_attachment(
        self, mock_ml_get, mock_ci_get, mock_ss_filter, mock_log_exc
    ):
        """
        ...
        self.assertEqual(
            missing_attachment_call[0][0],  # positional arg
            "Mailgun POST claimed to have %s attachments, but %s is missing",
        )
...
class RouteHandlerAccessTests(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.members = [
            {"address": a} for a in ["unittest@example.edu", cls.regular_member_address]
        ]
...
class RouteHandlerSuperSenderTests(RouteHandlerAccessTests):
    def test_from_is_supersender_sender_is_authorized(self):
        ...
        self.mock_ss.return_value.values_list.return_value = [
            self.regular_member_address
        ]
        self.post_body["sender"] = "Alt Email <{}>".format(alternate_email)
        self.post_body["from"] = "{} <{}>".format(
            from_display_name, self.regular_member_address
        )
...
class RouteHandlerReadOnlyListTests(RouteHandlerAccessTests):
    def test_read_only_list(self):
        ...
        self.assertEqual(
            bounce_back_template_arg, "mailgun/email/bounce_back_readonly_list.html"
        )
...
class RouteHandlerMembersOnlyListTests(RouteHandlerAccessTests):
    def test_members_list_sender_and_from_are_not_members(self):
        ...
        bounce_back_template_arg = self.mock_send_bounce.call_args[0][0]
        self.assertEqual(
            bounce_back_template_arg, "mailgun/email/bounce_back_not_subscribed.html"
        )
...
class RouteHandlerStaffOnlyListTests(RouteHandlerAccessTests):
    ...
    def test_staff_list_from_is_member_sender_is_authorized(self):
        ...
        self.post_body["sender"] = "Alt Email <{}>".format(alternate_email)
        self.post_body["from"] = "{} <{}>".format(
            from_display_name, self.staff_member_address
        )
...
class RouteHandlerRegressionTests(TestCase):
    ...
    def test_empty_body_html(self, mock_ml_get, mock_ci_get, mock_ss_filter):
        ...
        * do not have an html body
        ...
        # prep a MailingList mock
        members = [
            {"address": a} for a in ["unittest@example.edu", "student@example.edu"]
        ]
        ml = MagicMock(
            canvas_course_id=123,
            section_id=456,
    ...
    def test_multi_mailing_list_recipients_with_course_settings_none(
        self, mock_ml_get, mock_ci_get, mock_ss_filter
    ):
        """
        TLT-1943
    ...
    def test_multi_mailing_list_recipients_with_always_mail_staff_false(
        self, mock_ml_get, mock_ci_get, mock_ss_filter
    ):
        """
        TLT-1943
        ...
        # prep a MailingList mock
        members = [
            {"address": a} for a in ["student@example.edu", "unittest@example.edu"]
        ]
    ...
    def test_course_list_does_not_check_section_settings(
        self, mock_ml_get, mock_ci_get, mock_ss_filter
    ):
        """
        TLT-1943
    ...
    def test_multi_mailing_list_recipients_with_always_mail_staff_true(
        self, mock_ml_get, mock_ci_get, mock_ss_filter
    ):
        """
        TLT-2066
    ...
    def test_sender_receives_copy_when_always_mail_staff_false(
        self, mock_ml_get, mock_ci_get, mock_ss_filter
    ):
        """
        TLT-2960
    ...
    def test_sender_in_section_and_receives_single_mail_when_always_mail_staff_false(
        self, mock_ml_get, mock_ci_get, mock_ss_filter
    ):
        """
        TLT-2960
    ...
    def test_email_to_multiple_sections_when_always_mail_staff_false(
        self, mock_ml_get, mock_ci_get, mock_ss_filter
    ):
        """
        TLT-2960
        ...
        recipients = ", ".join(
            ["canvas-123-456@example.edu", "canvas-789-456@example.edu"]
        )
        post_body = {
            "sender": "Unit Test <teacher1@example.edu>",
            "recipient": recipients,
        ...
        send_mail_call = call(
            ...
            original_cc_address=[],
            original_to_address=[
                "canvas-123-456@example.edu",
                "canvas-789-456@example.edu",
            ],
            text="blah blah",
        )
        ml.send_mail.assert_has_calls([send_mail_call, send_mail_call])
    ...
    def test_bounce_loop_detection(
        self, mock_ml_get, mock_ci_get, mock_ss_filter, mock_send_bounce
    ):
        """
        TLT-2114
    ...
    def test_sender_display_name_from_field(
        self, mock_ml_get, mock_ci_get, mock_ss_filter, mock_get_name_for_email
    ):
        """
        TLT-2160
        ...
        self.assertEqual(
            ml.send_mail.call_args[0][0], self.user.get_full_name() + " via Canvas"
        )
    ...
    def test_sender_display_name_from_enrollment(
        self, mock_ml_get, mock_ci_get, mock_ss_filter, mock_get_name_for_email
    ):
        """
        TLT-2160
        ...
        self.assertEqual(
            mock_get_name_for_email.call_args,
...
class DecoratorTests(TestCase):
    ...
    def _test_authenticate_failure(self, request_post, mock_redirect):
        request = Bunch(POST=request_post)
        view = MagicMock(return_value="fake response")
        decorated = authenticate()(view)
        response = decorated(request)
    ...
    def test_authenticate_missing_timestamp(self, mock_redirect):
        self._test_authenticate_failure(
            {"token": self.POST["token"], "signature": self.POST["signature"]}
        )
    ...
    def test_authenticate_missing_token(self, mock_redirect):
        self._test_authenticate_failure(
            {"timestamp": self.POST["timestamp"], "signature": self.POST["signature"]}
        )

    @patch("mailgun.decorators.redirect")
    def test_authenticate_missing_signature(self, mock_redirect):
        self._test_authenticate_failure(
            {"timestamp": self.POST["timestamp"], "token": self.POST["token"]}
        )
