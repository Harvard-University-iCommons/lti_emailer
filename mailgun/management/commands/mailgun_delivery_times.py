...
class Command(BaseCommand):
    help = "Prints average and max delivery time for email sent in the past N days."
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=int,
            default=7,
            help="Number of days back to look at emails sent. (default: %(default)s)",
        )
    ...
    def handle(self, *args, **options):
        ...
        logger.info(
            "Pulling {} days of events for {}".format(
    ...
    def process_events(self, events):
        ...
        for message_id in times["accepted"]:
            if message_id in times["delivered"]:
                delivered_count += 1
                delivery_time = (
                    times["delivered"][message_id] - times["accepted"][message_id]
                )
